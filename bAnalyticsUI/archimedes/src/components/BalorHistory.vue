...
<template>
  <v-container fluid class="BalorJobHistory">
      ...
      <v-flex xs12>
        <v-card class="white">
          <v-card-title primary-title class="primary pt-2 pb-2">
            ...
            <v-spacer></v-spacer>
            <v-btn success router to="/Balor/NewBalor" class="white--text">+ NEW BALOR INPUT</v-btn>
          </v-card-title>
        </v-card>
      </v-flex>
    </v-layout>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.$store.commit('switchApp', {module: 'BALOR Job History'})
      this.getResults()
    },
    methods: {
      getResults () {
        userAppJobs(this.user, 'balor')
          .catch(err => {
