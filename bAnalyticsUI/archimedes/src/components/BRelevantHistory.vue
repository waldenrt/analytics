/**
* Created by jfullylove on 3/6/18.
*/

<template>
  <v-container fluid class="BRelevantJobHistory">
      ...
      <v-flex xs12>
        <v-card class="white">
          <v-card-title primary-title class="primary pt-2 pb-2">
            ...
            <v-spacer></v-spacer>
            <v-btn success router to="/BRelevant/NewBRelevant" class="white--text text-xs-left">+ NEW BRELEVANT INPUT</v-btn>
          </v-card-title>
        </v-card>
      </v-flex>
    </v-layout>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.$store.commit('switchApp', {module: 'b-Relevant Job History'})
      this.getResults()
    },
    methods: {
      getResults () {
        userAppJobs(this.user, 'BRelevant')
          .catch(err => {
            alert('Could not get User b-Relevant Job History results. ' + err.message.toString())
          })
          .then((response) => {
      ...
      addToJson () {
        ...
        for (let i = 0; i < this.jsonMsg.length; i++) {
          ...
          links[i].routeLink = '/BRelevant/Summary'
